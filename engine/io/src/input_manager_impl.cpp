...
void InputManagerImpl::initialize()
{
...
void InputManagerImpl::process_gamepad_added(EventManager* event_manager, int32_t device_index)
{
    
}

void InputManagerImpl::process_gamepad_removed(EventManager* event_manager, int32_t device_index)
{
    
}

void InputManagerImpl::process_joystick_added(EventManager* event_manager, int32_t device_index)
{
    
}

void InputManagerImpl::process_joystick_removed(EventManager* event_manager, int32_t device_index)
{
    
}

void InputManagerImpl::process_mouse_motion(EventManager* event_manager, int32_t xpos, int32_t ypos, int32_t xrel, int32_t yrel, bool relative)
{
...
void InputManagerImpl::process_mouse_button(EventManager* event_manager, uint8_t key, uint32_t state)
{
    if(m_default_player != UINT32_MAX)
    {
        auto& e = m_players.lookup(m_default_player);
...
void InputManagerImpl::process_mouse_wheel(EventManager* event_manager, uint32_t value)
{
    if(m_default_player != UINT32_MAX)
    {
        auto& e = m_players.lookup(m_default_player);
...
void InputManagerImpl::process_keyboard_button(EventManager* event_manager, uint32_t scan_code, uint32_t state)
{
    if(m_default_player != UINT32_MAX)
    {
        auto& e = m_players.lookup(m_default_player);
...
void InputManagerImpl::process_gamepad_axis(EventManager* event_manager, uint8_t axis, int16_t value, int32_t device_id)
{
...
void InputManagerImpl::process_gamepad_button(EventManager* event_manager, uint8_t button, uint8_t state, int32_t device_id)
{
    PlayerEntry* players = m_players.array();
...
void InputManagerImpl::process_joystick_axis(EventManager* event_manager, uint8_t axis, int32_t value, int32_t device_id)
{
    PlayerEntry* players = m_players.array();
...
void InputManagerImpl::process_joystick_button(EventManager* event_manager, uint8_t button, uint8_t state, int32_t device_id)
{
    PlayerEntry* players = m_players.array();
...
void InputManagerImpl::process_joystick_hat(EventManager* event_manager, uint8_t hat, uint8_t value, int32_t device_id)
{
    PlayerEntry* players = m_players.array();
...
uint32_t InputManagerImpl::create_player()
{
...
uint32_t InputManagerImpl::num_players()
{
    return m_players.size();
}

void InputManagerImpl::destroy_player(uint32_t player)
{
...
void InputManagerImpl::set_input_map(InputMap* map, uint32_t player)
{
    if(m_players.has(player))
    {
        auto& e = m_players.lookup(player);
        e.map = static_cast<InputMapImpl*>(map);
    }
}

uint32_t InputManagerImpl::assign_gamepad(int32_t device_index, uint32_t player)
{
    if(m_players.has(player))
    {
...
void InputManagerImpl::resign_gamepad(uint32_t handle, uint32_t player)
{
    if(m_players.has(player))
    {
...
uint32_t InputManagerImpl::assign_joystick(int32_t device_index, uint32_t player)
{
    return 0; // TODO: implement this!
}

void InputManagerImpl::resign_joystick(uint32_t handle, uint32_t player)
{
    // TODO: implement this!
}

uint32_t InputManagerImpl::num_gamepad_device()
{
    uint32_t count = 0;
...
uint32_t InputManagerImpl::num_joystick_device()
{
    uint32_t count = 0;
...
bool InputManagerImpl::keyboard_pressed(uint32_t code)
{
    return m_keyboard_states[code];
}

bool InputManagerImpl::mouse_pressed(uint32_t code)
{
    return SDL_GetMouseState(NULL, NULL) & SDL_BUTTON(code);
}

double InputManagerImpl::mouse_axis(uint32_t code)
{
    int32_t pos[2];
...
bool InputManagerImpl::gamepad_pressed(uint32_t code, uint32_t handle)
{
    if(m_gamepads.has(handle))
    {
        auto& e = m_gamepads.lookup(handle);
...
double InputManagerImpl::gamepad_axis(uint32_t code, uint32_t handle)
{
    if(m_gamepads.has(handle))
    {
        auto& e = m_gamepads.lookup(handle);
