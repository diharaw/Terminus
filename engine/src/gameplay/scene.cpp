...
{
	uint32_t front = _removal_queue.front();
	uint32_t back = _removal_queue.back();

	for (uint32_t _front = front; _front < back; _front++)
	{
		RemovedEntity& removed_entity = _removal_queue.entities[_front & DEFERRED_QUEUE_MASK];
		removed_entity.counter++;

		if (removed_entity.counter >= 2)
		{
			_mesh_pool.remove(removed_entity.entity);
			_removal_queue.pop_front();
		}
		else
			break;
	}
...
void Scene::destroy_entity(Entity& entity)
{
	...
	{
		_removal_queue.push_back(entity);

		_script_system.on_entity_destroyed(entity);
		_camera_system.on_entity_destroyed(entity);
		_transform_system.on_entity_destroyed(entity);
		_physics_system.on_entity_destroyed(entity);
		_render_system.on_entity_destroyed(entity);
