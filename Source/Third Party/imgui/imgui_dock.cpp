...
ImVec2 ImGui::GetCurrentDockSize()
{
	IM_ASSERT(g_dock.m_current);
	return g_dock.m_current->size;
}

void ImGui::SetCurrentDockInitialPosSize(ImVec2 pos, ImVec2 size)
{
	if (g_dock.m_current->secondPass)
	{
		g_dock.m_current->setPosSize(pos, size);
		g_dock.m_current->secondPass = false;
	}

	if (g_dock.m_current->firstPass)
	{
		g_dock.m_current->firstPass = false;
		g_dock.m_current->secondPass = true;
	}	
}

ImVec2 ImGui::GetCurrentDockPos()
{
	IM_ASSERT(g_dock.m_current);
	return g_dock.m_current->pos;
}